We’ll proceed **exactly as per your diagram** with a **Raw Hosted Repository**.

---

# 🧱 PHASE 4.1: CREATE RAW HOSTED REPOSITORY (NEXUS UI)

📍 **Where**: Nexus Web UI
📍 **Who**: Admin user
📍 **Goal**: Create artifact storage for build bundle (ZIP)

---

## 🔹 STEP 1: Open Nexus UI

Open browser:

```
http://<VM-IP>:8081
```

Login:

* Username: `admin`
* Password: *(from /nexus-data/admin.password)*

---

## 🔹 STEP 2: Go to Repositories

In Nexus UI:

1. Click ⚙️ **Settings**
2. Click **Repositories**
3. Click **Create repository**

---

## 🔹 STEP 3: Choose Repository Type

You’ll see many options.

👉 Select:

```
Raw (hosted)
```

Click **Next**

---

## 🔹 STEP 4: Repository Configuration

Fill **EXACTLY** like this:

### 🔸 Repository Name

```
employee-raw-repo
```

### 🔸 Online

✅ Checked

### 🔸 Deployment policy

```
Allow redeploy
```

### 🔸 Blob store

```
default
```

### 🔸 Strict Content Type Validation

❌ Disable (important for ZIP)

---

## 🔹 STEP 5: Save Repository

Click **Create repository**

---

## ✅ VERIFICATION

You should now see:

```
employee-raw-repo   (raw-hosted)
```

---

# 🎯 WHAT YOU JUST ACHIEVED

✔ Nexus artifact repository created
✔ Ready to store build bundles
✔ Matches diagram exactly

---

## 🧱 PHASE 4.2: CREATE BUILD ARTIFACT (ZIP)

We will:

1. Build Node.js app
2. Create production ZIP bundle
3. Upload ZIP to Nexus Raw repo
---

📍 **Where**: VM
📍 **Input**: GitHub source code
📍 **Output**: `employee-app.zip` (build artifact)

---

## 🔹 STEP 1: Go to Project Root

```bash
cd ~/employee-app
```

Verify:

```bash
ls
```

You should see:

```
backend  docker-compose.yml
```

---

## 🔹 STEP 2: CREATE PRODUCTION BUILD DIRECTORY

We never zip source directly.
We create a **clean build output**.

```bash
mkdir -p build/employee-app
```

---

## 🔹 STEP 3: COPY REQUIRED FILES INTO BUILD

```bash
cp -r backend build/employee-app/
cp docker-compose.yml build/employee-app/
```

Remove unnecessary files (if any):

```bash
rm -rf build/employee-app/backend/node_modules
```

📌 Node modules will be installed during runtime/build.

---

## 🔹 STEP 4: CREATE ZIP ARTIFACT

```bash
cd build
zip -r employee-app-1.0.0.zip employee-app
```

Verify:

```bash
ls
```

You must see:

```
employee-app-1.0.0.zip
```

---

# ✅ BUILD ARTIFACT READY

Artifact name:

```
employee-app-1.0.0.zip
```

This is what Nexus is meant to store.

---

# 🧱 PHASE 4.3: UPLOAD ARTIFACT TO NEXUS (RAW REPO)

📍 **Where**: VM
📍 **Target**: Nexus Raw Hosted Repo (`employee-raw-repo`)

---

## 🔹 STEP 5: UPLOAD ZIP TO NEXUS (CLI – BEST PRACTICE)

Use **curl** (industry standard).

```bash
curl -u admin:<NEXUS_PASSWORD> \
--upload-file employee-app-1.0.0.zip \
http://localhost:8081/repository/employee-raw-repo/employee-app/employee-app-1.0.0.zip
```

📌 Replace `<NEXUS_PASSWORD>` with actual password.

---

## 🔹 STEP 6: VERIFY IN NEXUS UI

1. Go to **Browse**
2. Select `employee-raw-repo`
3. You should see:

   ```
   employee-app/
     └── employee-app-1.0.0.zip
   ```

---

# 🎯 WHAT YOU ACHIEVED

✔ Build artifact created
✔ Artifact versioned
✔ Uploaded to Nexus
✔ GitHub → Build → Nexus completed

---

# ✅ HOW TO CHECK ZIP FILE IN NEXUS UI (RAW HOSTED REPO)

📍 **Where**: Browser
📍 **URL**: `http://<VM-IP>:8081`

---

## 🔹 STEP 1: Login to Nexus UI

* Username: `admin`
* Password: *(your Nexus password)*

---

## 🔹 STEP 2: Go to “Browse” (MOST IMPORTANT)

On the **left menu**:

👉 Click **Browse**

📌 This is where artifacts are visible (many people miss this).

---

## 🔹 STEP 3: Select Your Raw Repository

In **Browse**, you’ll see a list of repositories on the left panel.

👉 Click:

```
employee-raw-repo
```

---

## 🔹 STEP 4: Navigate Folder Path

Click through folders:

```
employee-app
```

You should see:

```
employee-app-1.0.0.zip
```

---

## 🔹 STEP 5: Confirm Artifact Details

Click on `employee-app-1.0.0.zip`

On the **right side**, you should see:

* Name
* Size
* Path
* Upload date

This confirms the artifact is **successfully uploaded**.

---

## 🧪 OPTIONAL: DOWNLOAD FROM NEXUS UI (TEST)

From the same screen:

* Click **Download**
* ZIP should download to your machine

📌 This proves Nexus is serving artifacts correctly.

---

## 🔴 IF YOU DO NOT SEE THE ZIP

### 1️⃣ Refresh browser (Ctrl + Shift + R)

### 2️⃣ Check correct repository

Make sure:

```
employee-raw-repo
```

not another repo.

### 3️⃣ Verify upload path

Run on VM:

```bash
curl -I http://localhost:8081/repository/employee-raw-repo/employee-app/employee-app-1.0.0.zip
```

Expected:

```
HTTP/1.1 200 OK
```

---
  
It will show you **ALL correct ways** to download the ZIP from **Nexus to local**, clearly separated so you can choose what you need.

---

# ✅ METHOD 1: DOWNLOAD FROM NEXUS UI (EASIEST)

📍 **Where**: Browser (your local machine)

### Steps:
1. Open Nexus UI:
   ```
   http://<VM-IP>:8081
   ```
2. Login as `admin`
3. Click **Browse**
4. Click repository:
   ```
   employee-raw-repo
   ```
5. Open folder:
   ```
   employee-app
   ```
6. Click:
   ```
   employee-app-1.0.0.zip
   ```
7. Click **Download**

✔ ZIP will be downloaded to your **local system (Downloads folder)**

---

# ✅ METHOD 2: DOWNLOAD USING `curl` (FROM VM OR ANY LINUX)

📍 **Where**: VM or Linux local machine

```bash
curl -u admin:<NEXUS_PASSWORD> \
-o employee-app-1.0.0.zip \
http://<VM-IP>:8081/repository/employee-raw-repo/employee-app/employee-app-1.0.0.zip
```

📌 Replace:
- `<VM-IP>`
- `<NEXUS_PASSWORD>`

### Verify:
```bash
ls -lh employee-app-1.0.0.zip
```

---

# ✅ METHOD 3: DOWNLOAD USING `wget`

```bash
wget --user=admin --password=<NEXUS_PASSWORD> \
http://<VM-IP>:8081/repository/employee-raw-repo/employee-app/employee-app-1.0.0.zip
```

---

# ✅ METHOD 4: DOWNLOAD TO LOCAL WINDOWS MACHINE

If your Nexus is public:

```powershell
curl.exe -u admin:<NEXUS_PASSWORD> `
-o employee-app-1.0.0.zip `
http://<VM-IP>:8081/repository/employee-raw-repo/employee-app/employee-app-1.0.0.zip
```

---

# 🧪 VERIFY ZIP CONTENT

```bash
unzip -l employee-app-1.0.0.zip
```

Expected:
```
employee-app/
├── backend/
├── docker-compose.yml
```

---

# 🎯 WHAT THIS MEANS IN YOUR PIPELINE

✔ Nexus is working  
✔ Artifact is downloadable  
✔ Ready for deployment  
✔ Matches enterprise DevOps flow  

---

# ✅ CORRECT WAY TO DOWNLOAD ZIP IN WINDOWS (POWER POWERSHELL)

## 🔴 DO **NOT** USE `-u` in PowerShell

---

## ✅ METHOD 1: PowerShell Native (RECOMMENDED)

### Run this **exact command**:

```powershell
$pair = "admin:Nubeera@2025"
$bytes = [System.Text.Encoding]::ASCII.GetBytes($pair)
$base64 = [Convert]::ToBase64String($bytes)

Invoke-WebRequest `
  -Uri "http://20.54.74.56:8081/repository/employee-raw-repo/employee-app/employee-app-1.0.0.zip" `
  -Headers @{ Authorization = "Basic $base64" } `
  -OutFile "employee-app-1.0.0.zip"
```

✔ This **will work 100%** in PowerShell.

---

## ✅ METHOD 2: Use `curl.exe` (ALSO EASY)

Windows has **real curl** as `curl.exe`.

```powershell
curl.exe -u admin:Nubeera@2025 `
  -o employee-app-1.0.0.zip `
  http://20.54.74.56:8081/repository/employee-raw-repo/employee-app/employee-app-1.0.0.zip
```

📌 **Important**: Use `curl.exe`, not `curl`.

---

## ✅ VERIFY DOWNLOAD

```powershell
dir employee-app-1.0.0.zip
```

---

## 🧪 TEST ZIP CONTENT

```powershell
tar -tf employee-app-1.0.0.zip
```

(or unzip via Explorer)

---

## 🎯 WHY YOUR COMMAND FAILED (FOR UNDERSTANDING)

| Issue     | Reason                     |
| --------- | -------------------------- |
| `curl`    | Alias to Invoke-WebRequest |
| `-u`      | Ambiguous parameter        |
| Backticks | OK, but alias broke it     |

---

## 🔐 SECURITY NOTE (IMPORTANT)

You exposed your Nexus password in chat.
👉 **Change Nexus admin password after this**.

---

# 🔜 NEXT PHASE (FINAL DEPLOYMENT)

## 🧱 PHASE 7: DEPLOY FROM NEXUS USING NGINX

We will:

1. Pull ZIP from Nexus
2. Extract it
3. Serve via **Nginx container**
4. Access app through Nginx

This completes your **full diagram pipeline**.

