
# 🚀 PHASE 5: DEPLOY APPLICATION USING NGINX (FROM NEXUS ARTIFACT)

📍 **Where**: VM
📍 **Source**: Nexus ZIP
📍 **Server**: Nginx (Docker container)
📍 **Result**: App served via Nginx

---

## 🧱 ARCHITECTURE (FINAL)

```
Nginx → Node.js API → PostgreSQL
     ↑
   Nexus ZIP
```

---

# 🔹 STEP 5.1: PREPARE DEPLOYMENT FOLDER

```bash
cd ~/nubeera
mkdir deploy
cd deploy
```

---

# 🔹 STEP 5.2: DOWNLOAD ZIP FROM NEXUS (VM)

```bash
curl -u admin:<NEXUS_PASSWORD> \
-o employee-app-1.0.0.zip \
http://localhost:8081/repository/employee-raw-repo/employee-app/employee-app-1.0.0.zip
```

---

# 🔹 STEP 5.3: EXTRACT ARTIFACT

```bash
unzip employee-app-1.0.0.zip
cd employee-app
```

You should see:

```
backend
docker-compose.yml
```

---

# 🔹 STEP 5.4: ADD NGINX (REVERSE PROXY)

### 📄 Create `nginx.conf`

```bash
nano nginx.conf
```

Paste:

```nginx
events {}

http {
  server {
    listen 80;

    location / {
      proxy_pass http://nodeapp:3000;
    }
  }
}
```

---

# 🔹 STEP 5.5: UPDATE docker-compose.yml

Edit:

```bash
nano docker-compose.yml
```

### ✅ Replace with this **FINAL VERSION**

```yaml
version: "3.8"

services:
  postgres:
    image: postgres:15
    container_name: employee-postgres
    environment:
      POSTGRES_DB: employeedb
      POSTGRES_USER: admin
      POSTGRES_PASSWORD: admin123
    volumes:
      - pgdata:/var/lib/postgresql/data

  nodeapp:
    build: ./backend
    container_name: employee-nodeapp
    depends_on:
      - postgres
    environment:
      DB_HOST: postgres
      DB_USER: admin
      DB_PASSWORD: admin123
      DB_NAME: employeedb
      DB_PORT: 5432

  nginx:
    image: nginx:latest
    container_name: employee-nginx
    ports:
      - "80:80"
    volumes:
      - ./nginx.conf:/etc/nginx/nginx.conf
    depends_on:
      - nodeapp

volumes:
  pgdata:
```

---

# 🔹 STEP 5.6: BUILD & RUN FULL STACK

```bash
docker-compose down
docker-compose up --build -d
```

---

# 🔹 STEP 5.7: VERIFY CONTAINERS

```bash
docker ps
```

You MUST see:

```
employee-postgres
employee-nodeapp
employee-nginx
```

---

# 🔹 STEP 5.8: ACCESS APPLICATION (FINAL)

🌐 Browser:

```
http://<VM-IP>
```

✔ Served via **Nginx**
✔ Node.js behind proxy
✔ PostgreSQL persistent
✔ Artifact-based deployment

---

# 🧪 FINAL VERIFICATION

Add employee in UI →
Check DB:

```bash
docker exec -it employee-postgres psql -U admin -d employeedb -c "SELECT * FROM employees;"
```

---

# 🎯 WHAT YOU HAVE ACHIEVED (BIG DEAL)

✔ Multi-container app
✔ GitHub source control
✔ Nexus artifact repository
✔ Versioned build ZIP
✔ Nginx deployment
✔ Enterprise DevOps pipeline

---

You’ve done an **end-to-end enterprise DevOps pipeline** 🚀👏
